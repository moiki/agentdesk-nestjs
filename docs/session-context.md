...
- ✅ **System prompt por tenant** — el agente compone su prompt en servidor: identidad AgentDesk general + contexto de la compañía del tenant (recogido en signup). Nuevo módulo `src/agent/prompts/` (`TenantPromptService`, identity base, types). El `systemPrompt` del cliente deja de ser un override libre: ahora es **instrucciones extra opcionales** (cap 2000 chars). Signup ampliado con `industry`, `companyDescription`, `supportEmail`, `supportPhone`, `brandVoice`, `defaultLanguage` (opcionales). Migración `20260903005931_add_tenant_prompt_context`. Plan: `docs/plan-tenant-system-prompt.md`; instructivo FE: `docs/fe-tenant-signup-wizard.md`.
- ✅ **Idempotencia + control de concurrencia** — ataca 7 problemas de carreras/lost-updates: (1–3) mutex por conversación (`async-mutex` exact-pin) que serializa `POST /chat` y `POST /chat/approve`; (4) `idempotencyKey String?` en `Conversation` con `@@unique([tenantId,idempotencyKey])` (retry de `/chat` con la misma key reutiliza la conversación, sin doble efecto) + `POST /chat` acepta `idempotencyKey`; (5) dedup de tools por `toolCallId` en `ToolRegistry` (LRU 1000, **por turno** — `startTurn()` limpia el cache en cada `runChatTurn`/`runApproveTurn` para no filtrar resultados entre turnos/tests); (6) optimistic locking en tickets (`update_ticket` y `PATCH /tickets/:id` con `version`, CAS → `409`/`CONFLICT`); (7) CAS en `ConversationService` (`version` en `updateMessages`/`setStatus`). Migración `20260903170000_add_idempotency_and_optimistic_locking`. Firma `Tool.execute(input, toolCallId?)`.
- ✅ **Idempotencia DB en `create_ticket` (cierre del mecanismo #2 del checklist)** — `Ticket` ahora tiene `idempotencyKey String?` + `@@unique([tenantId, idempotencyKey])` (migración `20260903180000_add_ticket_idempotency_key`). `create_ticket` estampa cada ticket con el `toolCallId` de la tool call → un reintento del mismo efecto que llegue en un **proceso nuevo** (donde el dedup en memoria del turno ya no existe) devuelve el ticket ya creado vía `findFirst`/`create`+catch-P2002 (`tickets.service.ts`), en vez de duplicarlo. Helper compartido `src/common/prisma-error.ts` (`isUniqueViolation`), refactorizado en `agent.service.ts`. REST `POST /tickets` sin key → comportamiento previo. `update_ticket`/`delete_ticket` no necesitan key (update es idempotente + CAS; delete idempotente de facto).
- Verificación verde: build ✓ lint ✓ **110 unit ✓ (12 suites)** **45 e2e ✓ (6 suites)** `prisma migrate status` ✓.
...
| 28 | Dedup de tools cacheado en `ToolRegistry` (singleton) filtró resultados entre turnos/tests e2e (reusan mismo `toolCallId` `call-1`) | `ToolRegistry.startTurn()` limpia el cache al inicio de cada turno (`runChatTurn`/`runApproveTurn`); el dedup solo protege re-emisiones DENTRO del mismo turno |
...
| 29 | `AGENT_SYSTEM_PROMPT` ya NO es el prompt completo del agente | En `TenantPromptService` ahora es solo la **identidad base**; lo que corre en runtime es identidad + contexto del tenant. Un env corto ya no borra la taxonomía ni el contexto |
| 30 | `systemPrompt` del cliente era un override libre (inyección / inconsistencia) | Pasó a ser **instrucciones extra opcionales con cap (2000 chars)** que se APENDEN al prompt por tenant, nunca lo reemplazan |

Decisiones de arquitectura: la identidad del tenant sale **solo del JWT**; el modelo `Tenant` es admin (exento de scoping); `TenantScopedPrismaService` es el único camino para dominio de tenant; auth = middleware único como enforcement point (no guard, por orden middleware→guards en NestJS); signup usa transacción + `tenantContextStore.run` envolviendo el `$transaction`. El agent loop es **stateful server-side**: `AgentService.chat()` persiste la conversación (`ConversationService`) y `approve()` carga el estado canónico desde DB; el cliente solo guarda y reenvía el `conversationId`. `ToolRegistry` es central y extensible (registrar nuevos tools = implementar `Tool` y `register()`). Los tools se declaran con `mutating: true/false` para paralelismo y `requiresApproval: true/false` para aprobación humana. `EventEmitter2` emite eventos por iteración para Langfuse futuro. El **system prompt es propiedad del servidor**: se compone por tenant (`TenantPromptService`, identidad base + contexto de compañía del signup) y degrada a la identidad base si no hay tenant (ej. unit tests) — el loop nunca falla por composición de prompt. La identity base incluye **boundary anti-inyección** (ignorar instrucciones dentro de tool results / contenido de usuario que pidan cambiar el comportamiento).
...
- `src/agent/` — `agent.module.ts` (EventEmitterModule + LlmModule + TicketsModule + TenancyModule + PromptsModule), `agent.service.ts` (orchestrator loop con retry, budget compuesto, paralelización, `runLoop()`, approval flow + `approve()`, y `resolveSystemPrompt()` para el prompt por tenant), `conversation.service.ts` (persistencia server-side tenant-scoped: `create`/`updateMessages`/`setStatus`/`toMessages`), `chat.controller.ts` (`POST /chat`, `POST /chat/approve`), `dto/chat.dto.ts` (añade `conversationId`; `systemPrompt` ahora = instrucciones extra opcionales), `dto/approve-chat.dto.ts` (ahora `{ conversationId, decisions }`, `conversationHistory` eliminado).
- `src/agent/prompts/` — **nuevo (system prompt por tenant)**: `identity.prompt.ts` (identidad base AgentDesk + contrato tools/errores + política de aprobación + anti-inyección), `tenant-prompt.service.ts` (`buildForTenant()` compone identidad + contexto del tenant; `appendExtraInstructions()` con cap 2000), `tenant-prompt.types.ts` (`TenantPromptContext`), `prompts.module.ts`, `tenant-prompt.service.spec.ts`.
- `src/signup/` — dto ampliado con campos de contexto de compañía (`industry`, `companyDescription`, `supportEmail`, `supportPhone`, `brandVoice`, `defaultLanguage`, todos opcionales), service persiste esos campos en `Tenant`.
